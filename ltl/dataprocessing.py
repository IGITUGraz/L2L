...
logger = logging.getLogger(u'ltl.dataprocessing')


def get_var_from_generations(traj, variable_name_full, status_interval=None):
    u"""
    ...
    NOTE: Unlike :func:`.get_var_from_runs`, we do not allow fetching partially from
    ...
    :Example Usage: To get the distribution parameters from each generation of a run of cross-
        ...
        :meth:`~.CrossEntropyOptimizer.post_process` of the
        :class:`~.CrossEntropyOptimizer`, we may extract the, the
    ...
    top_group = traj.f_load_child(u'results.generation_params', recursive=True, load_data=pypetconstants.LOAD_SKELETON)

    def get_generation_id(gen_name):
        return int(gen_name.rsplit(u'_', maxsplit=1)[-1])  # generation_XXXX
    ...
    for gen_group in top_group.f_iter_nodes(recursive=False):
        ...
        if status_interval and gen_counter % status_interval == 0:
            logger.info(u"Completed variable extraction of {} for {} generations".format(variable_name_full, gen_counter))
        gen_counter += 1
...
def get_var_from_runs(traj, variable_name_full, run_ids=None, with_ids=False, status_interval=None):
    u"""
    ...
    status_interval = int(status_interval) if status_interval else None
    top_group_name, variable_name = variable_name_full.split(u'.', maxsplit=1)
    allowed_top_group_names = set([u'results', u'parameters', u'derived_parameters'])
    assert top_group_name in allowed_top_group_names, \
        u"The name must be of the form <top_level_group>.<variable_name> where "\
        u"top_level_group is one of {}".format(allowed_top_group_names)
    ...
    try:
        ...
        for run_id in run_ids:
            ...
            if not isinstance(top_group, ParameterGroup):
                result_list.append(top_group.f_get(u'$set.$.' + variable_name,
            ...
            if status_interval and run_id % status_interval == 0:
                logger.info(u"Completed extraction of variable {} from {} runs.".format(variable_name_full, run_counter))
            run_counter += 1
    finally:
        traj.v_idx = old_vidx
    if with_ids:
...
def get_skeleton_traj(filename, name_or_index=-1):
    ...
    load_params_dict = {
        u'load_parameters':pypetconstants.LOAD_DATA,
        u'load_results':pypetconstants.LOAD_SKELETON,
    ...
    if isinstance(name_or_index, str):
        ...
        load_params_dict[u'index'] = index

    # Loading Trajectory from file.
    with timed(logger, u"Primary Loading of The HDF File"):
        traj.f_load(**load_params_dict)
    logger.info(u"Finished Primary Loading")
    return traj
...
def get_empty_traj(filename, name_or_index=-1):
    ...
    load_params_dict = {
        u'load_parameters':pypetconstants.LOAD_DATA,
        u'load_results':pypetconstants.LOAD_NOTHING,
        u'load_derived_parameters':pypetconstants.LOAD_NOTHING,
        u'force':True
    }
