...
def dict_to_list(input_dict, get_dict_spec=False):
    ...
    :param input_dict: The dictionary to be converted into a list. It is assumed that each key
      in the dicionary is a string and each value is either a scalar numerical value or an
      iterable.

    :param get_dict_spec: This is a flag that indicates whether one wants the dictionary
      specification tuple to be returned. This tuple is required to convert back from the
      dictionary to list. It is disabled by default
      ...
      Dict-Specification:
        ...
        key_type may be one of DictEntryType.Sequence or DictEntryType.Scalar. In case the object
        is a scalar, value_length is 1.
...
def list_to_dict(input_list, dict_spec):
    ...
    :param input_list: This is the list to be converted into a dict.
    :param dict_spec: This is the dict specification that conveys information regarding the dict
      to be converted to. See :meth:`.dict_to_list` for information about the dict specification.
    
    :returns: A Dictionary representing the list. Note that for valid behaviour, the list should
      have been generated by :meth:`.dict_to_list`. Moreover, This operation is not a perfect
      inverse of list_to_dict. While the types of individual elements are preserved, the types
      of the iterables inside which they reside are not. list_to_dict creates all iterables as
      python lists
    """
    cursor = 0
    return_dict = {}
...
def get_grouped_dict(dict_iter):
    ...
    :param dict_iter: dictionary iterator
