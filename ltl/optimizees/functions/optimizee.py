...
class FunctionGeneratorOptimizee(Optimizee):
    ...
    def __init__(self, traj, fg_instance, seed):
        ...
        traj.individual.f_add_parameter(u'seed', seed)
    ...
    def create_individual(self):
        ...
        return {u'coords': self.random_state.rand(self.dims) * (self.bound[1] - self.bound[0]) + self.bound[0]}

    def bounding_func(self, individual):
        u"""
        Bounds the individual within the required bounds via coordinate clipping
        """
        return {u'coords': np.clip(individual[u'coords'], a_min=self.bound[0], a_max=self.bound[1])}
