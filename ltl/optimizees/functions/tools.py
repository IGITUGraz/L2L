...
def plot(fn, random_state):
    ...
    Z = [fn.cost_function([x, y], random_state=random_state) for x, y in izip(XX.ravel(), YY.ravel())]
    ...
    ax.zaxis.set_major_formatter(FormatStrFormatter(u'%.02f'))
    W = np.where(Z == np.min(Z))
    ax.set(title=u'Min value is %.2f at (%.2f, %.2f)' % (np.min(Z), X[W[0]], Y[W[1]]))

    # Add a color bar which maps values to colors.
    fig.colorbar(surf, shrink=0.5, aspect=5)
    plt.savefig(u'function.png')
    plt.show()
