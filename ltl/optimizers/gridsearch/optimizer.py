...
class GridSearchOptimizer(Optimizer):
    ...
    def __init__(self, traj,
        ...
        super(GridSearchOptimizer, self).__init__(traj, optimizee_create_individual=optimizee_create_individual,
        ...
        assert set(sample_individual.keys()) == set(optimizee_param_grid.keys()), \
            u"The Parameters of optimizee_param_grid don't match those of the optimizee individual"
        ...
        for param_name, param_type, param_length in optimizee_individual_param_spec:
            ...
            if param_type == DictEntryType.Scalar:
                ...
                curr_param_list = np.meshgrid(*([curr_param_list] * param_length), indexing=u'ij')
    ...
    def post_process(self, traj, fitnesses_results):
        ...
        """
        logger.info(u'Finished Simulation')
        logger.info(u'-------------------')
        logger.info(u'')
        ...
        logger.info(u'Storing Results')
        logger.info(u'---------------')

        for run_idx, run_fitness, run_weighted_fitness in izip(run_idx_array, fitness_array, weighted_fitness_array):
            traj.v_idx = run_idx
            traj.results.f_add_result(u'$set.$.fitness', np.array(run_fitness))
            traj.results.f_add_result(u'$set.$.weighted_fitness', run_weighted_fitness)

        logger.info(u'Best Individual is:')
        logger.info(u'')
        ...
        for param_name, _, _ in self.optimizee_individual_dict_spec:
            logger.info(u'  %s: %s', param_name, individual[param_name])
            self.best_individual[param_name] = individual[param_name]

        self.best_fitness = fitness_array[max_fitness_indiv_index]
        logger.info(u'  with fitness: %s', fitness_array[max_fitness_indiv_index])
        logger.info(u'  with weighted fitness: %s', weighted_fitness_array[max_fitness_indiv_index])
    ...
    def end(self, traj):
        ...
        """
        traj.f_add_result(u'final_individual', self.best_individual)
        traj.f_add_result(u'final_fitness', self.best_fitness)
        traj.f_add_result(u'n_iteration', self.g)

        logger.info(u'x -------------------------------- x')
        logger.info(u'  Completed SUCCESSFUL Grid Search  ')
        logger.info(u'x -------------------------------- x')
