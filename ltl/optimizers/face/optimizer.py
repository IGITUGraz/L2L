...
logger = logging.getLogger(u"optimizers.face")

FACEParameters = namedtuple(u'FACEParameters',
...
class FACEOptimizer(Optimizer):
    ...
    def __init__(self, traj, optimizee_create_individual, optimizee_fitness_weights, parameters,
        ...
        if parameters.min_pop_size < 1:
            raise Exception(u"min_pop_size needs to be greater than 0")
        if parameters.max_pop_size < parameters.min_pop_size:
            raise Exception(u"max_pop_size needs to be greater or equal to min_pop_size")
        if parameters.n_elite > parameters.min_pop_size:
            raise Exception(u"n_elite exceeds min_pop_size")
        if parameters.temp_decay < 0 or parameters.temp_decay > 1:
            raise Exception(u"temp_decay not in range")
        if parameters.smoothing >= 1 or parameters.smoothing < 0:
            raise Exception(u"smoothing has to be in interval [0, 1)")
        if parameters.seed is None:
            raise Exception(u"The 'seed' must be set")

        # The following parameters are recorded
        traj.f_add_parameter(u'min_pop_size', parameters.min_pop_size,
                             comment=u'Number of minimal individuals simulated in each run')
        traj.f_add_parameter(u'max_pop_size', parameters.max_pop_size,
                             comment=u'Maximal individuals in population')
        traj.f_add_parameter(u'n_elite', parameters.n_elite,
                             comment=u'Number of individuals to be considered as elite')
        traj.f_add_parameter(u'n_iteration', parameters.n_iteration,
                             comment=u'Number of iterations to run')
        traj.f_add_parameter(u'n_expand', parameters.n_expand,
                             comment=u'Expanding of population size in case of FACE')
        traj.f_add_parameter(u'stop_criterion', parameters.stop_criterion,
                             comment=u'Stop if best individual reaches this fitness')
        traj.f_add_parameter(u'smoothing', parameters.smoothing,
                             comment=u'Weight of old parameters in smoothing')
        traj.f_add_parameter(u'temp_decay', parameters.temp_decay,
                             comment=u'Decay factor for temperature')
        traj.f_add_parameter(u'seed', np.uint32(parameters.seed),
                             comment=u'Random seed used by optimizer')
        ...
        temp_indiv, self.optimizee_individual_dict_spec = dict_to_list(self.optimizee_create_individual(),
                                                                       get_dict_spec=True)
        traj.f_add_derived_parameter(u'dimension', len(temp_indiv),
        ...
        current_eval_pop = [self.optimizee_create_individual() for _ in xrange(parameters.min_pop_size)]
    ...
    def get_params(self):
        ...
        param_dict[u'distribution'] = self.recorder_parameters.distribution.get_params()
        return param_dict
    ...
    def post_process(self, traj, fitnesses_results):
        ...
        logger.debug(u'  Calculated gamma: %.4f', self.gamma)
        ...
        traj.results.generation_params.f_add_result(generation_name + u'.g', self.g,
                                                    comment=u'The index of the evaluated generation')
        traj.results.generation_params.f_add_result(generation_name + u'.gamma', self.gamma,
                                                    comment=u'The fitness threshold inferred from the evaluated '
                                                            u'generation (This is used in sampling the next generation')
        traj.results.generation_params.f_add_result(generation_name + u'.T', self.T,
                                                    comment=u'Temperature used to select non-elite elements among the'
                                                            u'individuals of the evaluated generation')
        traj.results.generation_params.f_add_result(generation_name + u'.best_fitness_in_run', self.best_fitness_in_run,
                                                    comment=u'The highest fitness among the individuals in the '
                                                            u'evaluated generation')
        traj.results.generation_params.f_add_result(generation_name + u'.pop_size', self.pop_size,
                                                    comment=u'Population size')
        ...
        if self.best_fitness_in_run > previous_best_fitness or self.gamma > previous_gamma:
            ...
            logger.info(u'  FACE increase population size by %d', n_expand)
            self.pop_size += n_expand
        else:
            # Stop algorithm
            expand = False
            logger.warning(u'  Possibly diverged')
        ...
        for parameter_key, parameter_value in self.distribution_results.items():
            traj.results.generation_params.f_add_result(generation_name + u'.' + parameter_key, parameter_value)
    ...
    def end(self, traj):
        ...
        traj.f_add_result(u'n_iteration', self.g)

        # ------------ Finished all runs and print result --------------- #
        logger.info(u"-- End of (successful) FACE optimization --")
